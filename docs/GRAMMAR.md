# SPICE Netlist Grammar — ngspice, Xyce, LTspice
...
- 🔵 **ngspice-specific:** RKM notation (`2K7`, `4R7`) accepted only under
  `ngbehavior=lt` (LTspice compatibility mode) — this is likely the *real*
  LTspice number-parsing rule, worth confirming once LTspice docs arrive.
...
LTspice is in scope) very likely more. A dialect-agnostic core grammar must
...
LTspice reference material has not been provided yet. Placeholder based on
...
- LTspice is a SPICE3-derived proprietary simulator (Analog Devices/Linear
  ...
  best *proxy* for LTspice's actual number-parsing and behavioral-source
...
- LTspice has a native `.step` directive (closer to Xyce's `.STEP` than to
  ngspice's emulate-via-`.control` approach) — needs verification of exact
  syntax.
- LTspice ships its own component/model library and symbol format distinct
...
- Licensing note already discussed separately: parsing/tooling over
  LTspice's plain-text netlist format (not its binary or proprietary model
  library content) is not expected to raise EULA concerns, but that was
  general orientation, not confirmed legal advice.

**Action item:** once LTspice docs are added to the docs folder, repeat the
same extraction-agent process used for ngspice/Xyce and merge into this
section with the same 🟢/🔵/🟠/🟡-style tagging (adding a 🟣 LTSPICE tag).
